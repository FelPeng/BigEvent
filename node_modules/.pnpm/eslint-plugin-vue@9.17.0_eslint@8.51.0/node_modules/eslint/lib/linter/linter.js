...
function createMissingRuleMessage(ruleId) {
  return Object.prototype.hasOwnProperty.call(ruleReplacements.rules, ruleId)
    ? `Rule '${ruleId}' was removed and replaced by: ${ruleReplacements.rules[
        ruleId
      ].join(', ')}`
    ...
    if (match[0].endsWith('*/')) {
      retv = Object.assign(
        ...
        commentParser.parseListConfig(
          extractDirectiveComment(match[1]).directivePart
        )
      )
    }
  }
...
class Linter {
  ...
  _verifyWithFlatConfigArrayAndProcessor(
    ...
    try {
      ...
      // If the message includes a leading line number, strip it:
      const message = `Preprocessing error: ${ex.message
        .replace(/^line \d+:/iu, '')
        .trim()}`
          ...
          inlineConfigProblems.push(
            createLintingProblem({
              ...
              message: `'${sourceCode.text.slice(
                node.range[0],
                node.range[1]
              )}' has no effect because you have 'noInlineConfig' setting in ${
                options.warnInlineConfig
              }.`,
              loc: node.loc,
              severity: 1
