...
function createIndividualDirectivesRemoval(directives, commentToken) {
  ...
  return directives.map((directive) => {
    ...
    const regex = new RegExp(
      String.raw`(?:^|\s*,\s*)(?<quote>['"]?)${escapeRegExp(
        ruleId
      )}\k<quote>(?:\s*,\s*|$)`,
      'u'
    )
...
function createCommentRemoval(directives, commentToken) {
  ...
  return {
    description:
      ruleIds.length <= 2
        ? ruleIds.join(' or ')
        : `${ruleIds.slice(0, ruleIds.length - 1).join(', ')}, or ${
            ruleIds[ruleIds.length - 1]
          }`,
    fix: {
      range,
      text: ' '
...
function applyDirectives(options) {
  ...
  const processed = processUnusedDisableDirectives(
    unusedDisableDirectivesToReport
  )

  const unusedDisableDirectives = processed.map(
