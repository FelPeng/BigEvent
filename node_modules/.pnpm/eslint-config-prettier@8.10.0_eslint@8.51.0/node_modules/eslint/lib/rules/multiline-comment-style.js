  ...
  create(context) {
    ...
    function processBareBlockComment(comment) {
      const lines = comment.value
        .split(astUtils.LINEBREAK_MATCHER)
        .map((line) => line.replace(/^\s*$/u, ''))
      const leadingWhitespace = `${sourceCode.text.slice(
      ...
      return lines.map((line) => {
        ...
        if (lineOffset.length > leadingWhitespace.length) {
          return `${lineOffset.slice(
            leadingWhitespace.length - (offset.length + lineOffset.length)
          )}${lineContents}`
        }
              ...
              context.report({
                ...
                fix(fixer) {
                  ...
                  for (const [idx, line] of lines.entries()) {
                    ...
                    offset = `${commentTextPrefix.slice(
                      prefix.length
                    )}${initialOffset}`
